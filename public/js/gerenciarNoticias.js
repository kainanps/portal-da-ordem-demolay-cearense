...
function listar_noticia(pagina, qnt_result_pg){
    ...
    $.post('../php/listarTabelaNoticia.php', dados , function(retorna){
        //Subtitui o valor no seletor id="conteudoPage"
        $("#conteudoPage").html(retorna);
    });
}
function excluir_noticia(id_noticia, pag_noticia){
    var dados = {
        id_noticia: id_noticia
    }
    if(confirm("Tem certeza que deseja excluir?")){
        $.post('../php/excluiNoticia.php', dados , function(retorna){
            listar_noticia(pag_noticia, qnt_result_pg);
        });
    }
} 